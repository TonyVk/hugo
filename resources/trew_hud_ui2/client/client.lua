					...
					if kmh == 0.0 then
						...
						if not eng then
							if orgsusp == nil then
								...
								local nbr = 0.13
								for i=1, #Config.Zracni, 1 do
									if GetHashKey(Config.Zracni[i].auto) == GetEntityModel(vehica) then
										nbr = Config.Zracni[i].height
										break
									end
								end
								while nbr > susppr do
...
RegisterKeyMapping('+hud', 'Otvori hud', 'keyboard', 'i')
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	...
	if ESX.PlayerData.job and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'vlasnik') and 'society_' .. ESX.PlayerData.job.name == society then
		SendNUIMessage({ action = 'setMoney', id = 'society', value = money })
	end
end)
...
AddEventHandler('esx:setJob', function(job)
	...
	if ESX.PlayerData.job.label ==  ESX.PlayerData.job.grade_label then
		ime = ESX.PlayerData.job.grade_label
	else

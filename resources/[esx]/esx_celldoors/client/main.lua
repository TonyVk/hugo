		...
		for _,doorID in ipairs(Config.DoorList) do
			if doorID.doors then
				...
				if udalj < 5.0 then
					for k,v in ipairs(doorID.doors) do
						if not v.object or not DoesEntityExist(v.object) then
							v.object = GetClosestObjectOfType(v.objCoords, 1.0, v.hashKey, false, false, false)
						end
					end
				end
			else
				...
				if udalj < 30.0 then
					if not doorID.object or not DoesEntityExist(doorID.object) then
						doorID.object = GetClosestObjectOfType(doorID.objCoords, 30.0, doorID.hashKey, false, false, false)
					end
				end
			end
		end
